...
z.PdfSigner = class{
	...
	async sign(pdf, cypopt){
		...
		/** @type {forge_cert} */
		var cert = _this.loadP12cert(_this.opt.p12cert, _this.opt.pwd, _this.opt.certificate, _this.opt.privateKey, _this.opt.useP12);
		/** @type {Zga.CertsChain} */
		var cchain = null;
		if(cert){
			if(z.urlFetch){
	...
	loadP12cert(p12cert, pwd, certificate, privateKey, useP12 = true) {
		...
		if (useP12) {
			...
			// console.log("loadP12cert", certificate, privateKey);
		   // Convert inputs to PEM strings if needed
		   const pemCert = certificate;
		   const pemKey = privateKey;
		   console.log("loadP12cert:::", pemCert, pemKey);
	   
		   // Validate inputs
		//    if (!pemCert || !pemKey) {
		// 	 throw new Error("Both certificate and private key are required");
		//    }
		   // Parse PEMs using node-forge
		   try {
			 ...
			 _this.cchain = new z.CertsChain([cert]);
